 									...
 									sLoRa_OTAA *OTAA_Data, sLoRa_Message *Message_Rx, sSettings *LoRa_Settings)
{
	...
	if(*RFM_Command == NEW_RFM_COMMAND){
		if (LoRa_Settings->Confirm == 0) { // added 15.4.25 // sonst probleme bei lora.update(); bei abp ohne antwort
			...
			#endif
			*RFM_Command = NO_RFM_COMMAND; // added 15.4.25
			return; // added 15.4.25
        } // added 15.4.25
